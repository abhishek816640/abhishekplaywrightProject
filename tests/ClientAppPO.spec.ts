...
import dataSet from '../utils_ts/PlaceOrder.json';
...
customtest(`Client Po Login` , async ({page,testBasedata}) => {
  
   const username = testBasedata.username;
   const password = testBasedata.password;
   const productName = testBasedata.productName;
   const poManager = new POManager(page,productName);
   const products = page.locator(".card-body");
   const loginPage = poManager.getLoginPage();
   await loginPage.gotoLogin();
